...
const JobsList: React.FC<{ jobs?: Jobs; entity: Entity }> = ({
  jobs,
  entity,
}) => {
  const classes = useStyles();
...
export const WorkflowRunDetails: React.FC<{ entity: Entity }> = ({
  entity,
}) => {
  const { owner, repo } = useContext(GithubEntityContext);

  const id = "1"; // todo: should technically come from url/route...

  const hostname = "github.com"; // todo: read from entity yaml
  const details = useWorkflowRunsDetails({ hostname, id, owner, repo });
  const jobs = useWorkflowRunJobs({ hostname, id, owner, repo });
            ...
            <TableRow>
              ...
              <TableCell>
                {details.value?.html_url && (
                  <Link target={"_blank"} to={details.value.html_url}>
