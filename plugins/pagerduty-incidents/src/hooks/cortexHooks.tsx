...
export interface PluginUpdateFns {
  doAddSecret: (name: string, secret: string) => Promise<boolean>;
  doAddProxy: (
    name: string,
    tag: string,
    secretTag: string
...
export const usePluginUpdateFns = (): PluginUpdateFns => {
  ...
  const doAddSecret = useCallback(
    async (name: string, secret: string): Promise<boolean> => {
      if (!internalBaseUrl) {
        errorToast({
  ...
  const doAddProxy = useCallback(
    async (name: string, tag: string, secretTag: string): Promise<boolean> => {
      if (!internalBaseUrl) {
        errorToast({
